---
title: Research
---
<br>

Link to [Google Scholar Page](https://scholar.google.com/citations?user=h5ia4ZMAAAAJ&hl=en&oi=ao)


<br>

## Overview
---
Research in my lab encompasses a combination of field-based investigation and sample retrieval, molecular techniques, cultivation-based methods, and bioinformatic analyses. My research seeks to understand population-level dynamics of microorganisms within their environmental habitats to resolve how spatial, temporal, and biogeochemical features influence their ecology and evolution.

Much of my interests and focus are guided by these two questions:

- *What are the selective pressures influencing the genomic structures of microbial populations that shape their evolution and ecology under environmental stress?*

- *Are the processes of microbial ecosystems observed in their environmental context beneficial in an applied context – can we learn how to harness a metabolic process for bioremediation or other bio-adapted mechanisms?*
<br>
Though inevitable with all research, unexplored questions arise that lead to ever-evolving rabbit holes of projects. It's pushing in these new directions that lead to the most rewarding experiences.

<br>

<br>

## Current Research Projects
---
These are some active areas of research - prospective students in the lab should look here if they are interested in joining the lab.

#### <u>Estuaries</u>

Estuaries encompass highly dynamic and variable (micro)environments. Tidal estuaries, such as the Hudson River, are tightly linked to the shifting water properties due to daily tidal inundation and other runoff influences. Natural salinity gradients occur not only horizontally, but also vertically that can create a density stratified water column. The Hudson River serves as an ideal model for understanding broader dynamics of an estuarine system spanning hundreds of miles from New York Bay to Troy. Given the seasonal patterns of hydrodynamic inputs of freshwater and the distinct salinity zones, the Hudson River serves as a model template to investigate what drives ecological relationships of microorganisms that inhabit this dynamic ecosystem.

A major goal of this project is to establish regular intervals to collect samples in the hopes of understanding both long-term (seasonal) and micro-scale fluctuations in microbial community composition within estuarine environments at designated research stations. To provide a robust understanding of microbial habitability, sampling of both the water column and surface sediments will enable a deeper understanding of potential cross-feeding of key nutrients or “microbial seeding” during unfavorable environmental conditions. This high sampling opportunity will allow us to leverage a deep collection of spatio-temporal dynamics of microbial communities.

This overall effort is meant to be a long term endeavor that encompasses many research paths. Some of these initial paths include:

- *Creating a detailed biogeographic analysis of microbial communities, followed by informed phylogenetic and metabolic reconstructions of genomes*
- *Identify key microbial taxa and metabolisms to evaluate ecotype distributions and population dynamics*
- *Identify targets for cultivation efforts and physiological characterization - this includes potential members such as nitrifiers or other archaeal members*
- *Design meso/microcosm experiments to test relevant anthropogenic forces that can act upon a local estuary system*
<br>

<img src="Estuary_Diagram.gif" class="align-left" style="width:340px;"> <img src="Tidal_River.jpeg" class="align-right" style="width:320px;">








<br class="clear"> *Figures from [Crump & Bowen (2024)](https://www.annualreviews.org/content/journals/10.1146/annurev-marine-022123-101845) and [Scanes et al. (2017)](https://link.springer.com/chapter/10.1007/978-94-024-0990-1_2)*

<br>

#### <u>Mine Drainage</u>

While not as common on the east coast, abandoned mines can generate acidic waters as a result of dissolved sulfide or other metals-rich minerals. The Philips Mine in Garrison serves as a portal to understand what kind of microbial assemblages are shaped by these leftover tailings. Unique taxa typically contribute to low diversity, yet unique metabolic repertoire that must circumvent acidophilic conditions, high metals concentrations, and low nutrient availability. 

Some ares of interest include:

- *What are the fundamental ecological mechanisms that confer the growth of mine drainage microbial communities (iron-oxidizers, biofilms, syntrophic interactions)?*
- *What can we learn from the natural microbial communities that can inform bioremediation strategies?*

<br>

<div class="collage">
  <img src="Philips Mine.png" style="object-position: 10% 30%">
  <img src="PM_hole.jpeg">
  <img src="PM_Goop.jpeg">
  <img src="PM_tailings.jpeg">
</div>

<br class="clear"> 

*Site of Philips Mine and water pH values of drainage runoff from Gilchrest et al. (2009) (top left), abandoned mine entrance (top right), iron-precipitating biofilms at the beginning of the brook (bottom left), mine tailings with little to no vegetation growth (bottom right)*

<br class="clear">
#### <u>Population Biology & Comparative Genomics of Forgotten Microbes</u>

The field of bioinformatics has blossomed because of the ongoing advances in next-generation sequencing (NGS). In this vein, it has become incredibly cheap to sequence an entire genome and relatively cheap to sequence a metagenome of an environmental sample. Due to the overabundance of sequencing efforts, most studies only focus on a niche microorganism and then the rest of the data is left in the dregs! 

<img src="SNVs.png" class="align-left" style="width:415px;"> The focus on this work includes resolving the complete picture of the microbial community in these pre-existing datasets. These microbial members of interest could span from the rare biosphere (<1% abundance) to some of the key taxa that dominate their environments. This research topic will also involve discovering datasets that have not been fully exploited (shamelessly, this includes some of my own previous sequencing efforts). Some of the topics included in this work could be: read recruitment strategies, phylogenetic reconstructions, pangenomic analyses, and comparison of microdiversity metrics.

Some questions of interest:

- *What is the spatial / temporal resolution of distinct members across a defined environmental gradient (from the centimeter scale all the way to global distribution)?*

- *What genes are unique to defined ecotypes and what can the genetic variability of their genomes tell us about their ecological trajectory?*

<br> 

#### <u>Cultivation Experiments</u>

What good is all of this 'omics data if we don't understand the physiology of these microbes?

Contributing efforts towards the enrichment and isolation of previously uncultured bacteria and archaea is fundamental to better understanding a microbe's involvement in their ecosystem. This allows us to characterize the physiology of new strains, expand phylogenetic reconstructions of representative genomes, and probe designing genetic models within the lab. Cultivation is a long-term effort that significantly aids the field as a whole to contribute phenotypic observations along with our genomic investigations.

Direct questions or ideas will be relevant towards specific enrichments or isolates we are currently growing in the lab.

<div class="collage">
  <img src="Pressure_Curves.png">
  <img src="Glow_Worm.jpeg" style="object-position: 5% 5%">
</div>


<br>

<br>

---

---

<br>

<br>


## Previous Research Experiences:

Here are some examples of field work and research projects I have conducted or collaborated on.

---
#### <u>Deep Ocean Population Biology & High-Pressure Cultivation</u>

<img src="Ecotypes.png" class="align-right" style="width:300px;">
My postdoctoral work continued on the theme of subsurface microbial life, but instead into the marine biosphere. I worked on understanding the dispersal of ammonia-oxidizing archaea (AOA) throughout the water column especially in the deep ocean. This research helped to reveal how AOA are one of the most abundant microbial members below the mesopelagic zone and circumvent high hydrostatic pressure. Implementing efforts in long-read sequencing reconstructed some of the first genomes of AOA belonging to this part of the ocean. In particular, I focused on population biology metrics and microdiversity analysis of genetic information contained by their genomes and revealed by patterns of Single Nucleotide Variants (SNVs). This helped to shape how ecotypes were defined along the water column beyond physical location within the system, instead guided by genomic content. Further, this work shed light on specific genes under positive selection pressures that indicate local adaptations of unique populations within bathypelagic waters.

<br> 

**Paper coming soon**


<br class="clear">

<br>

Complementary to the above work, I also spent time testing cultivate isolated AOA strains within the lab under high hydrostatic pressures that mimicked the deep ocean. This involved using a set of custom-engineered pressure chambers and allowed us to ask physiological questions simply regarding the growth and rate of nitrification that may be taking place at different depths within the water column. As I have mentioned above, 'omics work alone will not paint the full picture - given that many proteins do not contain a known functional annotation. These experiments required non-conventional methods to observe growth of these microorganisms, and a style of thinking that I hope will carry on into my own cultivation work in my own lab.

<img src="Pressure_chamber.png" style="width:90px;"> <img src="Bulbs.jpg" style="width:195px;">  <img src="Pressure_Setup.png" style="width:345px;">  


<br>

---
#### <u>Microbial Community Dynamics & Lipid Biomarkers of the Samail Ophiolite</u>

<img src="Methano_pan.png" class="align-right" style="width:350px;"> Sites of serpentinization are targets for astrobiological investigation due to their relevancy in Mars' geologic history. If we can understand a modern analogue, we might begin to understand what is happening (or has happened) on other planetary bodies.

My work helped to deconvolute the role of methanogenic populations within the hyperalkaline subsurface fluids. Guided by a pangenomic approach, three unique populations were discovered - two of which existed in the same and highest pH (11) fluids. By understanding their genomic repertoire, it was revealed that the populations were undergoing niche differentiation in order to circumnavigate nutrient limitations in this extreme environment. Different strategies for alternative carbon source usage and biofilm adherence supported these unique populations.

[Paper linked here](https://www.frontiersin.org/journals/microbiology/articles/10.3389/fmicb.2023.1205558/full) 

<br>

Additional work was conducted to determine the biogeography of different microbial members across different pH and depth regimes within the subsurface. This approach resolved where specific microbial taxa are more likely to be found within the subsurface serpentinized system, and included looking at cDNA compositions in order to gauge which members were most active within the subsurface fluids. This work was paired with a lipidomic investigation to disentangle potential molecular biosignatures for these subsurface communities. Initial observations suggested that microbial communities in the most hyperalkaline fluids may have adapted their own unique lipid compositions in order to overcome osmotic stress induced from high pH conditions.

**Paper is currently in the works....**

<br>

---
#### <u>Spatial & Temporal constraints of microbes in boreholes of the Edgar Experimental Mine</u>

The Edgar Experimental Mine is a premier research and teaching portal into the subsurface. It was once an active economic mining site, but now is repurposed for use by the Colorado School Mines for demonstration and education purposes. We took advantage of this opportunity to explore the subsurface fluid hosted microbial communities within previously drilled boreholes. A "packer" device sealed the borehole to allow fluid to build up and allow for frequent sampling trips to extract for DNA and geochemistry. We discovered that microbial communities are highly dynamic at spatial scales <20 cm apart, and can display varying isolation/connectedness to surface meteoric input. 

This project was only an initial foray into the geobiological research that could be conducted at this site! 

[Paper linked here](https://journals.asm.org/doi/full/10.1128/spectrum.00631-21)

<div class="collage-grid-2">
  <img src="EdgarMine.jpeg" style="object-position: 5% 5%">
  <img src="Boreholes.jpeg">
</div>
<br>

---
#### <u>Yellowstone National Park Silicified Microbes</u>

Dr. Kalen Rasmussen wrote what I would consider to be the epitome of a geomicrobiology study. Titled "Living to Lithified", this work looks how microbial communities and lipid biomarkers shift along different degrees of silicification. I participated in field work sample collection and measurements as well as conducting a portion of the lipid analyses. 

[Paper linked here](https://onlinelibrary.wiley.com/doi/abs/10.1111/gbi.12620?casa_token=ROXKqh1DW0UAAAAA%3AX1WdvQMTLKZlYzmjhkloUZzwlJQQ_bO2_QewbPzPRN01qzN-6LgIJQJQf-lr1a477K4FAVR-WJKCoKwTfg)

<div class="collage-grid-2">
  <img src="Lithified.jpeg" style="object-position: 5% 5%">
  <img src="Kalen_Epic.jpeg">
</div>
